...
$(function() {
	...
	$(".num").blur("input propertychange", function() {
		...
		if(isNaN(num) || $cart.val().indexOf(".") != -1){
			...
			$cart.parents(".cart-list-li").find("span[name='succ_amount']").text(default_num);
			updateMoney(true);
			return;
		}
		if(min_buy != 0 && min_buy>num){
			...
			$cart.val(min_buy);
			$cart.parents(".cart-list-li").find("span[name='succ_amount']").text(min_buy);
			updateMoney(true);
			return;
		}else if (num == 0||num<0) {
			$cart.val(1);
			$cart.parents(".cart-list-li").find("span[name='succ_amount']").text(1);
			updateMoney(true);
			return;
		}
		...
		if (max_buy != 0 && num > max_buy) {
			// 限购
			$cart.val(max_buy);
			$cart.parents(".cart-list-li").find("span[name='succ_amount']").text(max_buy);
			updateMoney(true);
		...
		if (num > nummax) {
			$cart.val(nummax);
			$cart.parents(".cart-list-li").find("span[name='succ_amount']").text(nummax);
			updateMoney(true);
		...
		$.ajax({
			...
			success : function(res) {
				...
				$cart.val(num);
				$cart.parent().parent().find("span[name='succ_amount']").text(num);// 重新计算数量
				updateMoney(true);
			}
		});
	});
