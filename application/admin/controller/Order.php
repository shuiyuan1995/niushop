...
class Order extends BaseController
{
    ...
    public function orderDetail()
    {
        ...
        $arr = Db::name('ns_order')->where('order_id',$order_id)->find();
        $res = Db::name('c_card')->where('typeid',$order_id)->find();
        // var_dump($res);
        // exit;
        $this->assign('arr',$arr);
        $this->assign('res',$res);
  ...
  /**
    ...
    public function bargainOrderDetail(){
        ...
        $next_order = $order_service -> getOrderList(1,1,$conditions,'order_id asc');
        $this->assign('next_order',$next_order['data']);
    ...
    public function creditCardList()
    {
        ...
        if (request()->isAjax()) {
            ...
            $receiver_mobile = request()->post('receiver_mobile', '')
            ;
            $payment_type = request()->post('payment_type', 7);
            $shipping_type = request()->post('shipping_type', 0); //配送类型
            $order_type = request()->post('order_type', '1,3');
            $condition['order_type'] = array("in", $order_type); // 订单类型
...
//            halt($child_menu_list);
        ...
        $data = Db::name('ns_order')->where('order_id',$arr['typeid'])->find();
    ...
    public function doupdate()
    {
        ...
        $data = Db::name('c_card')->where('id',$_SESSION['id'])->update($arr);
        if ($data){
            $this->success('处理成功');
        }else{
            $this->error('处理失败');
