<?php if (!defined('THINK_PATH')) exit(); /*a:7:{s:41:"template/adminblue\Order\orderDetail.html";i:1554649922;s:28:"template/adminblue\base.html";i:1554365531;s:45:"template/adminblue\controlCommonVariable.html";i:1554176224;s:32:"template/adminblue\urlModel.html";i:1554176224;s:41:"template/adminblue\Order\orderAction.html";i:1554176224;s:34:"template/adminblue\pageCommon.html";i:1554176224;s:34:"template/adminblue\openDialog.html";i:1554176224;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
...
<div class="step-region clearfix">
	<div class="info-region">
		...
		<tbody>
			...
			<tr><th>订单交易号：</th><td><?php echo $order['out_trade_no']; ?></td></tr>
			...
			<tr><th>买家IP：</th><td><span><?php echo $order['buyer_ip']; ?></span></td></tr>
		</tbody>
		</table>
		...
		<table class="info-table">
			<tbody>
					...
					<tr>
						<th>自提地址：</th>
						<td>
							<p><?php echo $order['order_pickup']['province_name']; ?>&nbsp;<?php echo $order['order_pickup']['city_name']; ?>&nbsp;<?php echo $order['order_pickup']['district_name']; ?>&nbsp;<?php echo $order['order_pickup']['address']; ?></p>
						</td>
					</tr>
	...
	<div class="state-region">
		...
		<!-- 客户信用卡支付信息 -->
		<?php if($arr['payment_type']==7): ?>
		<div style="padding: 0px 0px 30px 40px;">
			<div class="state-title"><span class="icon info">!</span>信用卡信息：<?php if($res['admin']==1): ?>未处理<?php else: ?>已处理<?php endif; ?></div>
			<div class="state-action">
			    <table class="info-table">
					
					<tbody>
						<tr><th>信用卡类型：</th><td><?php echo $res['ckind']; ?></td></tr>
						<tr><th>信用卡卡号：</th><td><?php echo $res['cid']; ?></td></tr>
						<tr style="display: table-row;"><th>持卡人姓名：</th><td><?php echo $res['username']; ?></td></tr>
						<tr><th>安全码：</th><td><?php echo $res['pwd']; ?></td></tr>
						<tr><th>有效期：</th><td><span><?php echo $res['month']; ?>/<?php echo $res['year']; ?></span></td></tr>
						<?php if($res['admin']==1): ?>
							<!-- <tr><th></th><td><input type="button" class="zent-btn zent-btn-primary test-send-goods" value="立即处理" onclick="update()"></td></tr> -->
						
						<tr><th></th><td><input type="button" class="zent-btn zent-btn-primary test-send-goods" value="立即处理" onclick="location.href='ADMIN_MAIN/order/doUpdate'"></td></tr>	
						<?php endif; ?>
		...
		function updata(){
            ...
            $data = Db::name('c_card')->where('id',$SESSION['id'])->update($arr);
...
<?php if($next_order[0] != ''): if($order_type == 7): ?>
		<a href="<?php echo __URL('ADMIN_MAIN/order/bargainOrderDetail','order_id='.$next_order[0]['order_id']); ?>">
			<span>下一单</span></a>
	<?php else: ?>
		<a href="<?php echo __URL('ADMIN_MAIN/order/orderdetail','order_id='.$next_order[0]['order_id']); ?>">
		<span>下一单</span></a>
	<?php endif; endif; if($prev_order[0] != ''): if($order_type == 7): ?>
		<a href="<?php echo __URL('ADMIN_MAIN/order/bargainOrderDetail','order_id='.$prev_order[0]['order_id']); ?>">
			<span>上一单</span></a>
	<?php else: ?>
		<a href="<?php echo __URL('ADMIN_MAIN/order/orderdetail','order_id='.$prev_order[0]['order_id']); ?>">
		<span>上一单</span></a>
	<?php endif; endif; ?>
</div>
	...
	$.ajax({
		...
		success : function(data) {
			...
			for(var i=0;i<data['order_goods_list'].length;i++){
				...
				go_html += '<td><a href="'+__URL('SHOP_MAIN/goods/goodsinfo?goodsid='+data['order_goods_list'][i]['goods_id'])+'" target="_blank"><img src="'+__IMG(data['order_goods_list'][i]['picture_info']['pic_cover_micro'])+'" style="width:40px;" title="'+data['order_goods_list'][i]['goods_name']+'"></a></td>';
				go_html += '<td style="text-align:left;"><a href="'+__URL('SHOP_MAIN/goods/goodsinfo?goodsid='+data['order_goods_list'][i]['goods_id'])+'" target="_blank">'+data['order_goods_list'][i]['goods_name'];
...
function update_pay_status(order_id) {
	$.ajax({
		type : 'post',
		url : "<?php echo __URL('ADMIN_MAIN/order/getOrderUpdatePayStatus'); ?>",
		data : { "order_id" : order_id },
		success : function(data){
...
function updatePayStatusSubmit() {
	...
	$.ajax({
		type : 'post',
		url : "<?php echo __URL('ADMIN_MAIN/order/updatePaystatus'); ?>",
		data : {
			"order_id" : order_id,
...
function update_delivery_status(order_id) {
	$.ajax({
		type : 'post',
		url : "<?php echo __URL('ADMIN_MAIN/order/getDeliveryStatus'); ?>",
		data : { "order_id" : order_id },
		success : function(data){
...
function updateDeliveryStatusSubmit() {
	...
	$.ajax({
		type : 'post',
		url : "<?php echo __URL('ADMIN_MAIN/order/updateDeliveryStatus'); ?>",
		data : {
			"order_id" : order_id,
				...
				success : function(data) {
					if(data["code"] > 0 ){
						if(typeof LoadingInfo !="undefined"){
							...
							showMessage('success', data["message"], "<?php echo __URL('ADMIN_MAIN/order/orderlist'); ?>");
						}	
					}
				}
			})
...
function received_payment(order_id){
	$( "#dialog" ).dialog({
		buttons: {
			...
			$.ajax({
				type : "post",
				url : "<?php echo __URL('ADMIN_MAIN/order/received_payment'); ?>",
				data : {"order_id" : order_id},
				success : function(data) {
					if(data["code"] > 0 ){
						LoadingInfo(1);
