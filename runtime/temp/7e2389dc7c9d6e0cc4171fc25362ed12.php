<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:41:"template/wap\default_new\Login\login.html";i:1553151902;s:38:"template/wap\default_new\urlModel.html";i:1553151903;}*/ ?>
<!DOCTYPE html>
<html>
<head>
			...
			success : function(data) {
				 if(data["code"] > 0 ){
					...
					if(error_num != 0 && data['error_num'] >= error_num){
						$(".wap-captcha").show();
						$(".wap-captcha .verify_img").attr("src",'<?php echo __URL('SHOP_MAIN/captcha?tag=1'); ?>&send='+Math.random());
					}
					member_sub = false;
	...
	$.ajax({
		...
		success : function(data) {
			if(data["code"] > 0 ){
				...
				if(error_num != 0 && data['error_num'] >= error_num){
					$(".mobile-captcha").show();
					$(".mobile-captcha .verify_img").attr("src",'<?php echo __URL('SHOP_MAIN/captcha?tag=1'); ?>&send='+Math.random());
				}
			} 
		}
	});
	...
	<div class="log-wp">
		<div class="log-box">
				...
				<div id="nk_text1" style="display: block;">
					...
					<div class="log-cont wap-captcha" <?php if(!$is_need_verification): ?>style="display:none;"<?php endif; ?>>
				...
				<div id="nk_text2" style="display: none;">
					...
					<div class="nk-cont mobile-captcha" <?php if(!$is_need_verification): ?>style="display:none;"<?php endif; ?>>
