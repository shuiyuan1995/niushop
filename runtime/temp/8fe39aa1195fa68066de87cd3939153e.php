...
$(function(){
	...
	<div class="member-main" >
		<div class="box">
			<div class="tabmenu">
				<ul class="tab">
					<li class="active"><?php echo lang('member_my_virtual_code'); ?></li>
				</ul>
			</div>
			<?php if($total_count != ''): ?>
				<table>
		            <tr align="center">
		              <td><?php echo lang('member_commodity_name'); ?></td>              
		              <td><?php echo lang('virtual_code'); ?></td>
		              <td><?php echo lang('member_amount_money'); ?></td>
		              <td><?php echo lang('member_time'); ?></td>
		              <td><?php echo lang('expiration_time'); ?></td>
		            </tr>
		            <tbody id="list_content" class="close-order">
					<?php foreach($virtualList as $virtualList): ?>
						<tr align="center">
							<td><?php echo $virtualList['virtual_goods_name']; ?></td>
							<td><?php echo $virtualList['virtual_code']; ?></td>
							<td><?php echo $virtualList['money']; ?></td>
							<td><?php echo getTimeStampTurnTime($virtualList['create_time'] ); ?></td>
							<td><?php if($virtualList['end_time'] != 0): ?><?php echo getTimeStampTurnTime($virtualList['end_time'] ); else: ?>不限制<?php endif; ?></td>
						</tr>
					<?php endforeach; ?>
		            </tbody>
...
<div id="pagination" class="page" >
	...
	<div class="page-num fr">
		...
		<div id="page_list" style="float: left;<?php echo $path_info; ?>">
			...
			<!-- 总页数小于总页码时就都显示 -->
			<?php if($page_count <= $page_num): $__FOR_START_17906__=1;$__FOR_END_17906__=$page_count+1;for($i=$__FOR_START_17906__;$i < $__FOR_END_17906__;$i+=1){ if($i == $page): ?>
			...
			<?php elseif($page <= ($page_num-1)/2): $__FOR_START_25853__=1;$__FOR_END_25853__=$page_num+1;for($i=$__FOR_START_25853__;$i < $__FOR_END_25853__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info.'?page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } ?>
			<!-- 如果总页数等于当前页 或者 总页数小于当前页加上页码总数平均值显示 -->
			<?php elseif($page_count == $page or $page_count <= $page+($page_num-1)/2): $__FOR_START_15700__=$page_count-$page_num+1;$__FOR_END_15700__=$page_count+1;for($i=$__FOR_START_15700__;$i < $__FOR_END_15700__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info.'?page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } ?>
			<!-- 否则就正常显示 -->
			<?php else: $__FOR_START_29863__=$page-($page_num-1)/2;$__FOR_END_29863__=$page+($page_num-1)/2+1;for($i=$__FOR_START_29863__;$i < $__FOR_END_29863__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info.'?page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } endif; ?>
...
<?php endif; else: ?>
					<div>
						<div class="emptyFrame"> <i class="user_balance"></i><span><?php echo lang('you_do_not_have_any_virtual_code_yet'); ?>！</span> </div>
					</div>
			<?php endif; ?>
