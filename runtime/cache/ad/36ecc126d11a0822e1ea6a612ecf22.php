<?php
//000000000000a:3:{s:11:"orderAction";s:51492:"<style>
...
<!--修改支付方式模态-->
<div class="modal hide fade" id="update_pay_status" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="margin-left:-365px; width: 700px;">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
			...
			<div class="modal-body">
					<select id="payment_type" name="payment_type" class="select-common middle">
			...
			<div class="modal-footer">
				<input type="hidden" id="update_pay_status_id" />
				<button type="button" class="btn-common btn-big" onclick="updatePayStatusSubmit()">修改</button>
				<button type="button" class="btn-common-white btn-big" data-dismiss="modal">关闭</button>
			</div>
		</div>
	</div>
...
<!--修改发货状态-->
<div class="modal hide fade" id="update_delivery_status" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="margin-left:-365px; width: 700px;">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
			...
			<div class="modal-body">
				<select id="order_status" name="order_status" class="select-common middle">
					<option value="">全部</option>
			...
			<div class="modal-footer">
				<input type="hidden" id="update_delivery_status_id" />
				<button type="button" class="btn-common btn-big" onclick="">修改</button>
				<button type="button" class="btn-common-white btn-big" data-dismiss="modal">关闭</button>
			</div>
		</div>
	</div>
...
$(function() {
	...
	$.ajax({
		type : "post",
		url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/getprovince",
		dataType : "json",
...
function operation(operation_type, order_id){
	...
	}else if(operation_type == 'update_pay_status') {
		update_pay_status(order_id);//修改支付方式
	}else if(operation_type == 'update_delivery_status'){
		update_delivery_status(order_id);//修改发货状态
...
function orderDelivery(order_id){
	...
	$.ajax({
		type : "post",
		url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/orderdeliverydata",
		...
		success : function(data) {
			...
			for(var i=0;i<data['order_goods_list'].length;i++){
				...
				go_html += '<td><a href="'+__URL('http://127.0.0.1/xm/niushop01-master/index.php/goods/goodsinfo?goodsid='+data['order_goods_list'][i]['goods_id'])+'" target="_blank"><img src="'+__IMG(data['order_goods_list'][i]['picture_info']['pic_cover_micro'])+'" style="width:40px;" title="'+data['order_goods_list'][i]['goods_name']+'"></a></td>';
				go_html += '<td style="text-align:left;"><a href="'+__URL('http://127.0.0.1/xm/niushop01-master/index.php/goods/goodsinfo?goodsid='+data['order_goods_list'][i]['goods_id'])+'" target="_blank">'+data['order_goods_list'][i]['goods_name'];
...
function orderDeliverySubmit(){
	...
	$.ajax({
		type : "post",
		url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/orderdelivery",
	...
	$( "#dialog" ).dialog({
		buttons: {
			"确定": function() {
					$.ajax({
						type : "post",
						url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/orderofflinepay",
						data : {'order_id':order_id},
						success : function(data) {
							if (data["code"] > 0) {
								showMessage('success', data["message"],window.location.reload());
	...
	$( "#dialog" ).dialog({
		buttons: {
			"确定": function() {
					$.ajax({
						type : "post",
						url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/orderpresell/presellOrderOffLinePay",
...
function orderComplete(order_id){
	$.ajax({
		type : "post",
		url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/ordercomplete",
		data : {'order_id':order_id},
		success : function(data) {
...
function orderClose(order_id){
	$( "#dialog" ).dialog({
		buttons: {
			...
			$.ajax({
				type : "post",
				url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/orderclose",
				data : {"order_id" : order_id},
				success : function(data) {
					if(data["code"] > 0 ){
						LoadingInfo(1);
...
function modifyPrice(order_id,orderFreight){
	...
	$.ajax({
		type: "post",
		url: "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/getordergoods",
		data: { "order_id": order_id },
		dataType: "json",
		async: false,
		success: function (jsonData) {
			...
			for (var i = 0 ; i < jsonData.length; i++) {
				...
				str += "<a class='name' href="+__URL('http://127.0.0.1/xm/niushop01-master/index.php/wap?id='+jsonData[i].goods_id)+" target='_blank'>" + jsonData[i].goods_name + "</a>";
...
function updPrice(){
	...
	$.ajax({
		type: "post",
		url: "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/orderadjustmoney",
...
function orderSellerMemo(order_id){
	$.ajax({
		type : 'post',
		url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/getordersellermemo",
		data : { "order_id" : order_id },
...
function orderPickupSubmit(){
	...
	$.ajax({
		type: "post",
		url: "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/pickuporder",
...
function update_address(order_id){
	$.ajax({
		type : 'post',
		url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/getOrderUpdateAddress",
		data : { "order_id" : order_id },
		success : function(data){
...
//查询修改支付方式的信息
function update_pay_status(order_id) {
	$.ajax({
		type : 'post',
		url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/getOrderUpdatePayStatus",
		data : { "order_id" : order_id },
		success : function(data){

			$("#payment_type").val(data['payment_type']);
			$("#update_pay_status").modal('show');
			$("#update_pay_status .modal-body table tbody tr").remove();
			$("#update_pay_status_id").val(order_id);
		}
	});

}
//提交修改支付方式
function updatePayStatusSubmit() {
	var payment_type = $("#payment_type").val();
	var order_id = $("#update_pay_status_id").val();

	$.ajax({
		type : 'post',
		url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/updatePaystatus",
		data : {
			"order_id" : order_id,
		...
		success : function(data){
			if (data > 0) {
				showMessage('success', '修改支付方式成功',window.location.reload());
			}else{
				showMessage('error', '修改支付方式失败');
			}
		}
	});
}
//获取发货状态
function update_delivery_status(order_id) {
	$.ajax({
		type : 'post',
		url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/getDeliveryStatus",
		data : { "order_id" : order_id },
		success : function(data){
			$("#shipping_status").val(data['order_status']);
			$("#update_delivery_status").modal('show');
			$("#update_delivery_status .modal-body table tbody tr").remove();
			$("#update_delivery_status_id").val(order_id);
		}
	});
}
//提交修改发货状态
function updateDeliveryStatusSubmit() {
	var order_status = $("#order_status").val();
	var order_id = $("#update_delivery_status_id").val();

	$.ajax({
		type : 'post',
		url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/updateDeliveryStatus",
		data : {
			"order_id" : order_id,
		...
		success : function(data){
			if (data > 0) {
				showMessage('success', '修改发货方式成功',window.location.reload());
			}else{
				showMessage('error', '修改发货式失败');
			}
		}
	});
...
function updateAddressSubmit(){
	...
	$.ajax({
		type : 'post',
		url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/updateOrderAddress",
		data : {
			"order_id" : order_id,
...
function GetProvince() {
	...
	$.ajax({
		type : "post",
		url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/getcity",
		dataType : "json",
		data : {
			"province_id" : id
		},
...
function getSelCity() {
	...
	$.ajax({
		type : "post",
		url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/getdistrict",
		dataType : "json",
		data : {
			"city_id" : id
		},
...
function addMemoAjax(){
	...
	$.ajax({
		url: "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/addmemo",
...
function getdelivery(order_id){
	$.ajax({
		url: "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/orderTakeDelivery",
		data: { "order_id": order_id },
		type : "post",
		success: function(data) {
			if (data.code > 0) {
...
function delete_order(order_id){
	$( "#dialog" ).dialog({
		buttons: {
			...
			$.ajax({
				type : "post",
				url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/deleteOrder",
				...
				success : function(data) {
					if(data["code"] > 0 ){
						if(typeof LoadingInfo !="undefined"){
							...
							showMessage('success', data["message"], "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/orderlist");
						}	
					}
				}
			})
...
function updateExpressAjax(){
		...
		$.ajax({
			type : "post",
			url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/updateOrderExpress",
...
function o2o_delivery(order_id){
	...
	$.ajax({
		type : "post",
		url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/o2odeliverydata",
		data : {'order_id':order_id},
		success : function(data) {
			$("#delivery_order_id").val(order_id);
			var receiver_info = '收货信息：'+data['order_info']['address']+'&nbsp;'+data['order_info']['receiver_address']+'&nbsp;'+data['order_info']['receiver_name']+'&nbsp;'+data['order_info']['receiver_mobile'];//收货信息
...
function o2oDeliverySubmit(){
	...
	$.ajax({
		type : "post",
		url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/o2odelivery",
		data : {
			'order_id':order_id,
...
function presellStockingComplete(order_id){
	$( "#dialog" ).dialog({
		buttons: {
			...
			$.ajax({
				type : "post",
				url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/Orderpresell/orderStockingComplete",
				data : {"order_id" : order_id},
				success : function(data) {
					if(data["code"] > 0 ){
						LoadingInfo(1);
...
function received_payment(order_id){
	$( "#dialog" ).dialog({
		buttons: {
			...
			$.ajax({
				type : "post",
				url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/received_payment",
				data : {"order_id" : order_id},
				success : function(data) {
					if(data["code"] > 0 ){
						LoadingInfo(1);
...
</script>";s:16:"orderPrintAction";s:30762:"<style>
.cell .is_Increment{
	margin-left: 10px;
}	
.is_Increment{
...
.is_Increment + label{
    ...
    position: relative;
    top: 8px;
	background: url("/xm/niushop01-master/public/static/blue/img/un_checked.png") no-repeat;
}
.is_Increment:checked + label{
	background: url("/xm/niushop01-master/public/static/blue/img/checked.png") no-repeat;
}
	...
	<div class="modal-body" style="height: 282px; overflow: auto;">
		<form class="form-horizontal">
			<div class="control-group">
				...
				<span><a href="http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/express/expresscompany" class="goToConfigure" style="text-decoration: underline;">前去配置</a></span>
			</div>
		</form> 
		<div class="ordercontent">
			<div class="data-body-head">
...
function showInvoice() {
	...
	$.ajax({
		url: "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/getOrderInvoiceView",
		data: { "ids": strIDs },
		async: false, // 让它同步执行
		dataType: "json",
	...
	$.ajax({
		type: "post",
		url: "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/getshopinfo",
		dataType: "json",
		data: "oper=getType",
...
function invoicePrinPreview(ids) {
	...
	if (ids != "") {
		$("#invoicePrintingURL").attr("href", __URL("http://127.0.0.1/xm/niushop01-master/index.php/admin/order/printdeliverypreview?order_ids=" + ids + "&ShopName=" + ShopName + ""));
...
function printPreview() {
	...
	for (var i = 0; i < checks.length; i++) {
		...
		if (check == true) {
			...
			$.ajax({
				url: "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/printpreviewvalidate",
	...
	if (printQueue.length > 0) {
		$("#expressSinglePrintURl").attr("href", __URL("http://127.0.0.1/xm/niushop01-master/index.php/admin/order/printexpresspreview?expressIDs=" + printQueue + "&sid="+$("#expressTemplate").val()));
...
function printShipping(){
	...
	$.ajax({
		type : "post",
		url: "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/getShippingList",
...
function printpreviewOfInvoice(){
	...
	if (order_ids != "") {
		window.open('http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/printpreviewOfInvoice&order_ids='+order_ids +'');
	}
}
...
function showExpress() {
	...
	$("#print_select_ids").val(strIDs);
	$.ajax({
		url: "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/getorderexpresspreview",
		data: { "ids": strIDs },
		async: false, // 让它同步执行
		dataType: "json",
		success: function (data) {
			...
			if(data.length > 0){
				for(var i = 0; i < data.length; i++){
					...
					if(item['goods_array'].length > 1){
						...
						str += '"><img src="/xm/niushop01-master/public/static/blue/img/more.png" alt=""></a></td>';
					}
...
function expressPrinPreview(ids) {
	...
	if(expressno_is_empty){
		$.ajax({
			type : "post",
			url : 'http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/addTmpExpressInformation',
		...
		if (print_order_ids != "") {
			window.open(__URL("http://127.0.0.1/xm/niushop01-master/index.php/admin/order/printexpresspreview?print_order_ids=" + print_order_ids));
		}
	}
}  
...
function showNotShippingOrderList(){
	...
	$.ajax({
		type : "post",
		url: "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/getNotshippedOrderList",
		data: { "ids": strIDs },
		async: false, // 让它同步执行
		dataType: "json",
		success: function (data) {
...
$("#PrintOrder").click(function(){
	...
	if (BatchSend.length == 0) {
		...
		window.open(__URL("http://127.0.0.1/xm/niushop01-master/index.php/admin/order/printOrder?print_order_ids=" + BatchSend.toString()));
	}
})
</script>";s:17:"orderRefundAction";s:10288:"<link rel="stylesheet" href="/xm/niushop01-master/public/admin/css/order_refunds.css">
...
<script src="/xm/niushop01-master/public/admin/js/order/order_refunds.js"></script>
<script>
...
function refuseRefundType(type){
	...
	if(type == 1){
		refund_url = "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/ordergoodsrefuseonce";
	} else{
		refund_url = "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/ordergoodsrefuseforever";
	}
	$.ajax({
		type : "post",
		url : refund_url,
...
function orderGoodsConfirmRecieve(order_id,order_goods_id){
	...
	$.ajax({
		type : "post",
		url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/getOrderGoodsDetialAjax",
...
function confirm_receipt(){
	...
	$.ajax({
		type : "post",
		url : "http://127.0.0.1/xm/niushop01-master/index.php?s=/admin/order/ordergoodsconfirmrecieve",
