...
$(function() {
	...
	$(".js-save").click(function(){
		...
		if(validation()){
			...
			var request_url = __URL(ADMINMAIN + "/config/addWapCustomTemplate");

			if(parseInt($("#hidden_id").val()) > 0) request_url = __URL(ADMINMAIN + "/config/updateWapCustomTemplate");
				...
				success : function(res){
					...
					if(res.code>0){
						...
						if(parseInt($("#hidden_id").val()) == 0){
							...
							setTimeout(function(){
								location.href = __URL(ADMINMAIN + "/config/wapCustomTemplateList");
							},1500);
	...
	if(!empty(template_data)){
		...
		for(var i=0;i<template_data.length;i++){
			...
			if(control_name == controlList.GoodsSearch){
				...
				additional_attr += 'style="padding: ' + title.padding + 'px 0;background:url(' + __URL(title.bg_img) + ') 50% / 100% no-repeat ' + title.bg_color + ';"';
				...
				additional_attr = 'style="padding: ' + notice.padding + 'px 0;background:url(' + __URL(notice.bg_img) + ') 50% / 100% no-repeat ' + notice.bg_color + ';"';
				...
				additional_attr += 'style="padding: ' + text_navigation.padding + 'px 0;background:url(' + __URL(text_navigation.bg_img) + ') 50% / 100% no-repeat ' + text_navigation.bg_color + ';"';
		...
		if(res.code){
			...
			switch(getCustom().attr("data-custom-flag")){
				...
				case controlList.Notice:
					...
					getCustom().css({"background" : "url(" + __IMG(res.data) + ") no-repeat center/100% "});
	...
	html += '<div class="control-edit background">';
		...
		html += '<div class="add-img">';
			...
			if(!empty(bg_img)){
				...
				html += '<div class="img-block" style="display:block;"><img id="img_' + flag + '" style="max-height:100%;" src="' + __URL(bg_img) + '"></div>';
