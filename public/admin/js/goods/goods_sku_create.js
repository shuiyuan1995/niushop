 ...
 * ---------------------------------------------- 官方网址:
 * https://www.51jiyan.com/ 这不是一个自由软件！您只能在不用于商业目的的前提下对程序代码进行修改和使用。
...
function editSkuData(){
	...
	if($specObj.length > 0){
		...
		for(var i=0;i<$specObj.length;i++){
			...
			if($(".js-sku-list-popup li:visible:eq(0):not(.add-sku)").length>0){
				if(first_curr_li == null) first_curr_li = $(".js-sku-list-popup li:visible:eq(0):not(.add-sku)");
			}
			var spec_value = eval(curr_li.attr("data-spec-value-json"));
			var spec_value_id_array = new Array();
			var is_update_img = 0;
			curr_li.children("span:last").text("[" + curr_spec.value.length + "/" + curr_li.attr("data-spec-value-length") + "]");
			...
			for(var j=0;j<curr_spec.value.length;j++){
				...
				for(var k=0;k<spec_value.length;k++){
					//二次修改颜色、图片路径
					if(spec_value[k].spec_value_id == curr_spec.value[j].spec_value_id){
						spec_value[k].spec_value_data = curr_spec.value[j].spec_value_data;
						if(curr_spec.value[0].spec_show_type == 3 && spec_value[k].spec_value_data != ""){
							is_update_img++;
						}
					}
				}
			...
			curr_li.attr("data-show-type",curr_spec.value[0].spec_show_type);
			// curr_li.attr("data-spec-value-json",JSON.stringify(spec_value));
			if(curr_spec.value[0].spec_show_type == 3) curr_li.attr("data-is-update-img",is_update_img);
		}
		
		if(first_curr_li != null){
			//当前有第一个规格，触发第一个规格的点击事件
			first_curr_li.click();
		}else{
			//如果没有规格，则清空右侧数据
			$(".edit-sku-popup-body article>div.sku-value .empty-info").text("请选择左侧规格列表").show().siblings().hide();
			$(".edit-sku-popup-body article>div.sku-display-mode .empty-info").text("请选择上方的规格值").show().nextAll().hide();
		}
	}
...
function updateSpecObjData(){
	for(var i = 0 ; i <$specObj.length; i++ ){
		//编辑是显示所选的规格按钮
		//防重复
		if($(".sku-picture-span[spec_id='" + $specObj[i]["spec_id"] + "']").length == 0){
...
//								html += '<input type="hidden" id="goods_sku'+index+'" value="'+curr_spec_value.spec_value_data+'" >';
								...
								// html += '<input class="input-file" name="file_upload" id="skuPreviewUploadImg'+curr_spec_value.spec_value_id+'" type="file" onchange="skuPreviewUploadImg(this);" title="上传" data-spec-value-id="'+curr_spec_value.spec_value_id+'" data-spec-id="'+curr_spec.spec_id+'">';
								html += '</div>';
								break;
						}
