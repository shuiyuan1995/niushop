...
class NsGoodsViewModel extends BaseModel
{
     ...
     * @return unknown
     */
    public function getPintuanGoodsViewList($page_index, $page_size, $condition, $order)
    {
        $queryList = $this->getPintuanGoodsViewQuery($page_index, $page_size, $condition, $order);
        $queryCount = $this->getPintuanGoodsrViewCount($condition);
        $list = $this->setReturnList($queryList, $queryCount, $page_size);
        return $list;
    }
     ...
     * @return \data\model\multitype:number
     */
    public function getPintuanGoodsViewQuery($page_index, $page_size, $condition, $order)
    {
        $viewObj = $this->alias('ng')
            ->join('ns_goods_category ngc', 'ng.category_id = ngc.category_id', 'left')
            ->join('ns_goods_brand ngb', 'ng.brand_id = ngb.brand_id', 'left')
            ->join('sys_album_picture sap', 'ng.picture = sap.pic_id', 'left')
            ->join('ns_shop nss', 'ng.shop_id = nss.shop_id', 'left')
            ->join('ns_promotion_tuangou npg', 'ng.goods_id = npg.goods_id', 'left')
            ->field('ng.goods_id, ng.goods_name, ng.shop_id, ng.category_id, ng.brand_id, ng.group_id_array,
             ng.promotion_type, ng.goods_type, ng.market_price, ng.price, ng.promotion_price,
            ng.cost_price, ng.point_exchange_type, ng.point_exchange, ng.give_point,
            ng.is_member_discount, ng.shipping_fee, ng.shipping_fee_id, ng.stock, ng.max_buy,
            ...
            ng.description, ng.QRcode, ng.code, ng.is_stock_visible, ng.is_hot, ng.is_recommend,
            ng.is_new, ng.is_pre_sale, ng.is_bill, ng.state, ng.sale_date, ng.create_time,
            ng.update_time, ng.sort, ng.real_sales, ngb.brand_name, ngb.brand_pic, ngc.category_id, ngc.category_name, sap.pic_cover_micro,sap.pic_cover_mid,sap.pic_cover_small,nss.shop_name,nss.shop_type,sap.pic_id,sap.upload_type, sap.domain, sap.bucket, npg.tuangou_money , npg.tuangou_num, npg.tuangou_type, npg.tuangou_content_json, npg.is_open, npg.is_show');
        $list = $this->viewPageQuery($viewObj, $page_index, $page_size, $condition, $order);
        if (! empty($list)) {
            $goods_group_model = new NsGoodsGroupModel();
            $goods_sku = new NsGoodsSkuModel();
     ...
     * @return \data\model\unknown
     */
    public function getPintuanGoodsrViewCount($condition)
    {
        $viewObj = $this->alias('ng')
            ->join('ns_goods_category ngc', 'ng.category_id = ngc.category_id', 'left')
            ->join('ns_goods_brand ngb', 'ng.brand_id = ngb.brand_id', 'left')
     ...
     */
    public function getBargainGoodsViewList($page_index, $page_size, $condition, $order)
    {
        $queryList = $this->getBargainGoodsViewQuery($page_index, $page_size, $condition, $order);
        $queryCount = $this->getBargainGoodsrViewCount($condition);
        $list = $this->setReturnList($queryList, $queryCount, $page_size);
        return $list;
    }
     ...
     * @return \data\model\multitype:number
     */
    public function getBargainGoodsViewQuery($page_index, $page_size, $condition, $order)
    {
        $viewObj = $this->alias('ng')
        ->join('ns_goods_category ngc', 'ng.category_id = ngc.category_id', 'left')
        ->join('ns_goods_brand ngb', 'ng.brand_id = ngb.brand_id', 'left')
        ->join('sys_album_picture sap', 'ng.picture = sap.pic_id', 'left')
        ->join('ns_shop nss', 'ng.shop_id = nss.shop_id', 'left')
        ->join('ns_promotion_bargain_goods npbg', 'ng.goods_id = npbg.goods_id' )
        ->field('ng.goods_id, ng.goods_name, ng.shop_id, ng.category_id, ng.brand_id, ng.group_id_array,
             ng.promotion_type, ng.goods_type, ng.market_price, ng.price, ng.promotion_price,
            ng.cost_price, ng.point_exchange_type, ng.point_exchange, ng.give_point,
            ng.is_member_discount, ng.shipping_fee, ng.shipping_fee_id, ng.stock, ng.max_buy,
            ...
            npbg.bargain_id, npbg.status, npbg.start_time, npbg.end_time, npbg.fictitious_sales, npbg.sales, npbg.partake_number,
            npbg.bargain_min_rate, npbg.one_min_rate, npbg.one_max_rate, npbg.bargain_min_number');
        $list = $this->viewPageQuery($viewObj, $page_index, $page_size, $condition, $order);
        if (! empty($list)) {
            $goods_group_model = new NsGoodsGroupModel();
            $goods_sku = new NsGoodsSkuModel();
     ...
     * @return \data\model\unknown
     */
    public function getBargainGoodsrViewCount($condition)
    {
        $viewObj = $this->alias('ng')
        ->join('ns_goods_category ngc', 'ng.category_id = ngc.category_id', 'left')
        ->join('ns_goods_brand ngb', 'ng.brand_id = ngb.brand_id', 'left')
        ->join('sys_album_picture sap', 'ng.picture = sap.pic_id', 'left')
        ->join('ns_promotion_bargain_goods npbg', 'ng.goods_id = npbg.goods_id', 'left')
        ->field('ng.goods_id');
        $count = $this->viewCount($viewObj, $condition);
        return $count;
    }
