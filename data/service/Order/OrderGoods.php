...
class OrderGoods extends BaseService
{
     ...
     */
    public function addBragainOrderGoods($order_id, $goods_sku_list, $adjust_money = 0)
    {
        $this->order_goods->startTrans();
        try {
            $err = 0;
            ...
            foreach ($goods_sku_list_array as $k => $goods_sku_array) {
                ...
                $goods_sku_info = $goods_sku_model->getInfo([
                    'sku_id' => $goods_sku[0]
                ], 'sku_id,goods_id,cost_price,stock,sku_name,attr_value_items,price');
                ...
                $sku_price = $goods_sku_info["price"]; 
    ...
    public function addComboPackageOrderGoods($order_id, $goods_sku_list, $adjust_money = 0)
    {
        ...
        try {
            ...
            foreach ($goods_sku_list_array as $k => $goods_sku_array) {
				...
				//非砍价订单执行库存减少
                $order_model = new NsOrderModel();
                $order_info = $order_model->getInfo(['order_id'=>$order_id],"order_type");
                if($order_info['order_type'] != 7){
                	$goods_calculate = new GoodsCalculate();
                	$goods_calculate->subGoodsStock($goods_sku_info['goods_id'], $goods_sku_info['sku_id'], $goods_sku[1], '');
                }
    ...
    public function orderGoodsRefundMoney($order_goods_id)
    {
        ...
        // 如果订单是预售订单的话重新计算订单项金额
        if($order_other_pay_money["order_type"] == 6){
            $money = $this->getPresellOrderGoodsRefundMoney($order_goods_info['order_id']);
        }
    ...
    public function orderGoodsRefundBalance($order_goods_id)
    {
        ...
        // 如果是预售订单则重新获取
        if($order_other_pay_money["order_type"] == 6){
            $refund_balance = $this->getPresellOrderGoodsRefundBanlance($order_goods_info['order_id']);
        }
...
//         $order_goods_info['refund_real_money'] = $this->orderGoodsRefundMoney($order_goods_id);
     ...
     * 获取预售订单可退金额
     */
    public function getPresellOrderGoodsRefundMoney($order_id){
        $ns_order = new NsOrderModel();
        $ns_presell_order = new NsOrderPresellModel();
        
        $money = 0; // 应退金额
        
        $presell_order_info = $ns_presell_order -> getInfo(["relate_id" => $order_id], "is_full_payment,point_money,presell_pay,platform_money");
        $order_info = $ns_order -> getInfo(["order_id" => $order_id], "order_money,point_money,user_platform_money,tax_money,shipping_money,pay_money");
     ...
     * 获取预售订单可退余额
     */
    public function getPresellOrderGoodsRefundBanlance($order_id){
        $ns_order = new NsOrderModel();
        $ns_presell_order = new NsOrderPresellModel();
