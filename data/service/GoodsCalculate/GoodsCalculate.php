...
class GoodsCalculate extends BaseService
{
    ...
    public function subGoodsStock($goods_id, $sku_id, $num, $cost_price)
    {
        ...
        try {
            $retval_sku=$goods_sku_model->where(['sku_id' => $sku_id, "stock"=>array("egt", $num)])->setDec('stock', $num);
            $retval=$goods_model->where(['goods_id' => $goods_id, "stock"=>array("egt", $num)])->setDec('stock', $num);
            if($retval_sku<=0 || $retval<=0){
