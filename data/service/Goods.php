...
//             array_multisort($arrSort[$sort['field']], SORT_ASC, $spec_list); 
    ...
    public function syncUserCart($uid)
    {
        ...
        if (! empty($cart_goods_list)) {
            foreach ($cart_goods_list as $k => $v) {
                ...
                if (empty($cart_query)) {
                    // 获取商品sku信息
                    $this->addCart($uid, $this->instance_id, $web_info['title'], $v["goods_id"], $goods_info["goods_name"], $v["sku_id"], $sku_info["sku_name"], $price, $v["num"], $goods_info["picture"], 0, $ip);
                } else {
                    $is_have = true;
    ...
    public function getGoodsCoupon($goods_id, $uid)
    {
        ...
        $coupon_type_id_list = $coupon_type->getCouponTypeListByGoods($goods_id);
        $coupon_list = array();
